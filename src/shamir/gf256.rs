//! Implementation of Galois Field GF(2^8) operations
//! Used for finite field arithmetic in Shamir's Secret Sharing

// Lookup tables for fast GF(2^8) arithmetic
const LOG_TABLE: [u8; 256] = [
    ,
];

const EXP_TABLE: [u8; 256] = [
    ,
];

/// GF256 represents an element in the Galois Field GF(2^8)
#[derive(Debug, Copy, Clone, PartialEq, Eq)]
pub struct GF256 {
    value: u8,
}

impl GF256 {
    /// Create a new GF256 element
    pub fn new(value: u8) -> Self {
        Self { value }
    }

    /// Get the value of the element
    pub fn value(&self) -> u8 {
        self.value
    }
}

// Implementation of addition (which is XOR in GF(2^8))
impl std::ops::Add for GF256 {
    type Output = Self;

    fn add(self, other: Self) -> Self {
        Self {
            value: self.value ^ other.value,
        }
    }
}

// Implementation of subtraction (which is also XOR in GF(2^8))
impl std::ops::Sub for GF256 {
    type Output = Self;

    fn sub(self, other: Self) -> Self {
        Self {
            value: self.value ^ other.value,
        }
    }
}

// Implementation of multiplication
impl std::ops::Mul for GF256 {
    type Output = Self;

    fn mul(self, other: Self) -> Self {
        if self.value == 0 || other.value == 0 {
            return Self { value: 0 };
        }

        let log_a = LOG_TABLE[self.value as usize];
        let log_b = LOG_TABLE[other.value as usize];
        let sum = (log_a as u16 + log_b as u16) % 255;
        
        Self {
            value: EXP_TABLE[sum as usize],
        }
    }
}

// Implementation of division
impl std::ops::Div for GF256 {
    type Output = Self;

    fn div(self, other: Self) -> Self {
        // Division by zero is undefined in finite fields
        if other.value == 0 {
            panic!("Division by zero in GF(256)");
        }
        
        if self.value == 0 {
            return Self { value: 0 };
        }

        let log_a = LOG_TABLE[self.value as usize];
        let log_b = LOG_TABLE[other.value as usize];
        let diff = (log_a as i16 - log_b as i16 + 255) % 255;
        
        Self {
            value: EXP_TABLE[diff as usize],
        }
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_addition() {
        let a = GF256::new(0x53);
        let b = GF256::new(0xCA);
        let result = a + b;
        assert_eq!(result.value(), 0x53 ^ 0xCA);
    }

    #[test]
    fn test_subtraction() {
        let a = GF256::new(0x53);
        let b = GF256::new(0xCA);
        let result = a - b;
        // Subtraction is the same as addition in GF(2^8)
        assert_eq!(result.value(), 0x53 ^ 0xCA);
    }

    #[test]
    fn test_multiplication() {
        let a = GF256::new(0x53);
        let b = GF256::new(0xCA);
        let result = a * b;
        
        // Manual calculation for verification
        let log_a = LOG_TABLE[0x53];
        let log_b = LOG_TABLE[0xCA];
        let expected = EXP_TABLE[((log_a as u16 + log_b as u16) % 255) as usize];
        
        assert_eq!(result.value(), expected);
    }

    #[test]
    fn test_division() {
        let a = GF256::new(0x53);
        let b = GF256::new(0xCA);
        let result = a / b;
        
        // Manual calculation for verification
        let log_a = LOG_TABLE[0x53];
        let log_b = LOG_TABLE[0xCA];
        let expected = EXP_TABLE[((log_a as i16 - log_b as i16 + 255) % 255) as usize];
        
        assert_eq!(result.value(), expected);
    }

    #[test]
    fn test_multiply_by_zero() {
        let a = GF256::new(0x53);
        let b = GF256::new(0);
        let result = a * b;
        assert_eq!(result.value(), 0);
    }

    #[test]
    fn test_zero_divided_by_nonzero() {
        let a = GF256::new(0);
        let b = GF256::new(0xCA);
        let result = a / b;
        assert_eq!(result.value(), 0);
    }

    #[test]
    #[should_panic(expected = "Division by zero")]
    fn test_division_by_zero() {
        let a = GF256::new(0x53);
        let b = GF256::new(0);
        let _ = a / b;
    }
}